...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        # Matrix seq_len x d_model, maximun length of sentences is seq_len
        # Create a matrix of shape (seq_len, d_model)
        pe = torch.zeros(seq_len, d_model)

        # Positional enconding, based in two formulas: 
        #   first:  PE(pos, 2i) = sin(pos/100000 * 2i/d_model)
        #   secon:  PE(pos, 2i + 1) = cos(pos/100000 * 2i/d_model)
        # first for odd positions, second for odd positions
        ...
        # It's not a learn parameter, but I want to save it when save the model.
        # so it will be save with the state (dict) of the model
        self.register_buffer('pe', pe)

    def forward(self, x):
        """add this positional encoding to every word inside to the particular sentence
        """
...
class LayerNormalization(nn.Module):
    ...
    def __init__(self, eps: float = 10 ** -6) -> None:
        ...
        super().__init__()
        self.eps = eps
        self.alpha = nn.Parameter(torch.ones(1)) # Miltiplied
...
class FeedForwardBlock(nn.Module):
    ...
    def forward(self, x):
        # x's travel
        # (batch, seq_len, d_model) --> (batch, seq_len, d_ff) --> (batch, sqe_len, d_model)
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        ...
        # each h Head: will see the full sentece (seq_len, d_k), means each word in the sentence 
        # but only the smaller part of the embedding
        ...
        # (bacth, h, seq_len, d_k) --> (batch, seq_len, h, d_k) --> (batch, seq_len, d_model) 
        # contiguous: transform the shape of tensor, put the memory to be contiguous??
        #  do it in place, 
        # self.h * self.d_k => d_model size
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, src_mask):
        # Order of executions:
        # 1.  self.self_attention_block is executed, return Wo matrix
        # 2.  norm layer is executed, return a statistical LayerNormalization 
        # Objetive: combine in the Norm layer [x from Embedding] + [Wo matrix from Attention Block]
        x = self.residual_connections[0](x, lambda x: self.self_attention_block(x, x, x, src_mask))
        # I asume that the same occuer here but with Fee Forward layer.
...
class ProjectionLayer(nn.Module):
    """This is the proyection Linear layer:
...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, 
    ...
    # Crete the encoder and the decoder
